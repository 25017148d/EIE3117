...
export const useRouteStore = defineStore("route", () => {
  ...
  function normalizeRoute(route: Route) {
    ...
    return {
      ...
      isBooked: Boolean(route.isBooked),
      passengers,
      passengerDetails,
    };
  }
  ...
  async function fetchMyBookings() {
    const resp = await axios.get(`${API_BASE}/routes/my-bookings/`, {
      headers: authHeaders(),
    });
  ...
  async function fetchMyRoutes() {
    const resp = await axios.get(`${API_BASE}/routes/my-routes/`, {
      headers: authHeaders(),
    });
  ...
  async function addRoute(
    ...
    const resp = await axios.post(`${API_BASE}/routes/`, routeData, {
      headers: authHeaders(),
    });
    const normalized = normalizeRoute(resp.data);
    mergeRoute(normalized);
  ...
  async function cancelBooking(routeId: string) {
    const resp = await axios.delete(`${API_BASE}/routes/${routeId}/book/`, {
      headers: authHeaders(),
    });
    mergeRoute(resp.data);
    await fetchMyBookings();
