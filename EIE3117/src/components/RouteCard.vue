...
const isBooked = computed(() => props.route.isBooked ?? false);
...
<template>
  <div
    class="bg-white overflow-hidden shadow rounded-lg hover:shadow-md transition-shadow duration-200 border border-gray-100 flex flex-col h-full"
  >
      ...
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center space-x-3">
          <img
            ...
            :src="
              route.driverAvatar ||
              `https://i.pravatar.cc/150?u=${route.driverId}`
            "
            alt=""
          />
          <div>
        ...
        <div
          class="bg-blue-50 text-blue-700 text-xs px-2 py-1 rounded-full font-medium"
        >
    ...
    <div
      class="bg-gray-50 px-4 py-4 sm:px-6 flex justify-between items-center border-t border-gray-100"
    >
      <button
        @click="handleView"
        class="text-sm font-medium text-primary hover:text-blue-600 transition-colors"
      >
        View Details
      </button>
      ...
      <div v-if="isPassenger">
        <button
          v-if="!isBooked && !isFull"
          @click="handleBook"
          class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-primary hover:bg-blue-600 shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-colors"
        >
          Book Seat
        </button>
        <button
      ...
      <div v-else-if="isDriver && isMyRoute">
        <span
          class="text-xs font-medium text-green-600 bg-green-100 px-2 py-1 rounded"
          >My Route</span
        >
      </div>
    </div>
